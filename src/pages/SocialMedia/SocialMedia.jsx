...
const AnimatedBox = ({ children }) => {
  ...
  useEffect(() => {
    control.start('visible');  // Start the animation on load
  }, [control]);
  ...
  useEffect(() => {
    if (inView) {
      ...
      control.start('hidden');  // Start the animation on load
    }
  }, [control, inView]);
...
const SocialMedia = () => {
  ...
  // Define functions to calculate dynamic dimensions based on viewport width
  const calculateInstagramWidth = () => {
    if (viewportWidth >= 1200) {
      return 348.45;
    } else if (viewportWidth >= 768) {
      return 330; // Adjust this value based on your design
    } else {
      return 360; // Adjust this value based on your design
    }
  };
  ...
  const calculateInstagramReelWidth = () => {
    if (viewportWidth >= 1200) {
      ...
      return 400; // Adjust this value based on your design
    } else {
      return 360; // Adjust this value based on your design
    }
  };
